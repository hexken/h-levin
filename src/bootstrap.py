    ...
    def solve_uniform_online(self, planner, nn_model, shuffle=False):
        ...
        while len(current_solved_puzzles) < self._number_problems:
            ...
            for name, state in self._states:
                ...
                if (
                    len(batch_problems) < self._batch_size and last_puzzle != name
                ):  # and self._number_problems - len(current_solved_puzzles) > self._batch_size:
                    continue

                with ProcessPoolExecutor(max_workers=self._ncpus) as executor:
                    args = (
                        (state, name, budget, nn_model)
