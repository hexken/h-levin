...
class PUCTTreeNode:
    ...
    def get_uct_values(self, max_q, min_q):
        ...
        for a, q in self._Q.items():
            normalized_Q[a] = (q + self._virtual_loss[a] - min_q) / (
                self._virtual_loss[a] + max_q - min_q
            )

        uct_values = {}
        for a in self._actions:
            uct_values[a] = normalized_Q[a] - self.c * self._P[a] * (
                np.math.sqrt(self._N_total) / (1 + self._N[a])
            )
...
class PUCT:
    ...
    def _evaluate(self, nodes, actions, children):
        ...
        if self._use_learned_heuristic:
            _, action_probs, predicted_h = self._nn_model.predict(
                np.array(children_image)
            )
        else:
    ...
    def search_for_learning(self, data):
        ...
        if self._use_learned_heuristic:
            _, action_probs, _ = self._nn_model.predict(
                np.array([state.get_image_representation()])
            )
        else:
            _, action_probs = self._nn_model.predict(
                np.array([state.get_image_representation()])
            )
