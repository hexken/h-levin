...
def main():
    ...
    puzzle_files = [
        f
        for f in listdir(join(path_to_problems, folder))
        if isfile(join(path_to_problems, folder, f))
    ]
    ...
    for i in range(len(random_indices)):
        if i < len(random_indices) / 2:
            states_4x4[puzzle_files_4x4[random_indices[i]]].save_state(
                join(path_to_problems, folder_test, puzzle_files_4x4[random_indices[i]])
            )
        else:
            states_4x4[puzzle_files_4x4[random_indices[i]]].save_state(
                join(
                    path_to_problems, folder_train, puzzle_files_4x4[random_indices[i]]
                )
            )
    ...
    for i in range(len(random_indices)):
        if i < len(random_indices) / 2:
            states_5x5[puzzle_files_5x5[random_indices[i]]].save_state(
                join(path_to_problems, folder_test, puzzle_files_5x5[random_indices[i]])
            )
        else:
            states_5x5[puzzle_files_5x5[random_indices[i]]].save_state(
                join(
                    path_to_problems, folder_train, puzzle_files_5x5[random_indices[i]]
                )
            )
