...
class HeuristicConvNet(tf.keras.Model):
    ...
    def __init__(self, kernel_size, filters, number_actions, reg_const=0.001):
        ...
        self.conv1 = tf.keras.layers.Conv2D(
            filters, kernel_size, name="conv1", activation="relu", dtype="float64"
        )
        ...
        self.conv2 = tf.keras.layers.Conv2D(
            filters, kernel_size, name="conv2", activation="relu", dtype="float64"
        )
        ...
        self.dense1 = tf.keras.layers.Dense(
            128, name="dense1", activation="relu", dtype="float64"
        )
...
class TwoHeadedConvNet(tf.keras.Model):
    ...
    def __init__(
        self, kernel_size, filters, number_actions, loss_name, reg_const=0.001
    ):
        tf.keras.backend.set_floatx("float64")
        ...
        self.dense11 = tf.keras.layers.Dense(
            128, name="dense11", activation="relu", dtype="float64"
        )
        self.dense12 = tf.keras.layers.Dense(
            number_actions, name="dense12", dtype="float64"
        )

        # Heuristic value
        self.dense21 = tf.keras.layers.Dense(
            128, name="dense21", activation="relu", dtype="float64"
        )
...
class ConvNet(tf.keras.Model):
    ...
    def __init__(
        ...
        self.dense2 = tf.keras.layers.Dense(
            number_actions, name="dense2", dtype="float64"
        )
